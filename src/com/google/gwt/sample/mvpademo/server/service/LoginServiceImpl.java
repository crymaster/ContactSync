...
public class LoginServiceImpl extends RemoteServiceServlet implements
	...
	public CSUser login(CSUser csUser) {
		...
		csUser.setVersion(0);
		User user = CSConverter.toUser(csUser);
		if (dao.addUser(user).equals("")) {
			System.out.println("[loginService] User already registered");
			user = dao.getByUserName(csUser.getEmailAddress());
			System.out.println("[loginService] User name: " + user.getName()
	...
	public Integer push(CSUser csUser) {
		System.out.println("PUSH\nUser Name: " + csUser.getEmailAddress());
		...
		Contact contact;
		Phone phone;
		User user = dao.getByUserName(csUser.getEmailAddress());
		if (user == null)
			return -1;
